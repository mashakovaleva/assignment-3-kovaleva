...
if __name__ == '__main__':
    base_dir = os.path.dirname(__file__)  # folder: baselines/ruwordnet
    data_dir = os.path.join(base_dir, '..', 'data')
    data_dir = os.path.abspath(data_dir)

    os.makedirs(data_dir, exist_ok=True)

    db_path = os.path.join(data_dir, 'ruwordnet_no_lemmas.db')
    xml_path = os.path.join(data_dir, 'ruwordnet_xml')  # <-- extracted XML dir

    RuWordnet(
        db_path=db_path,
        ruwordnet_path=xml_path,     # Use forward slashes or os.path.join
        with_lemmas=False,          # optional switch
    )
    print("RuWordNet database created")
